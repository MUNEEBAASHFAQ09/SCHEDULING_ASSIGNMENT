...
def virtual_round_robin():
    ...
    #LOADING FEATURE FOR MY PROGRAM TO MAKE IT ATTRACTIVE
    loading_page()
    print('\n')
    os.system("clear")
          ...
          for i in range (count):
            ...
            if(LIST_OF_PROCESSES[i].get_remaining_brust_time()>0):
                ...
                if (LIST_OF_PROCESSES[i].get_remaining_brust_time()>quantum_time):
                    departure_time=departure_time+quantum_time
                    LIST_OF_PROCESSES[i].set_remaining_brust_time(LIST_OF_PROCESSES[i].get_remaining_brust_time()-quantum_time)
                    
                else:
                    departure_time=departure_time+LIST_OF_PROCESSES[i].get_remaining_brust_time()
                    LIST_OF_PROCESSES[i].set_waiting_time(departure_time-LIST_OF_PROCESSES[i].get_brust_time()-LIST_OF_PROCESSES[i].get_arrival_time())
...
##                   READY_QUEUE.append(LIST_OF_PROCESSES[i])
##                elif(LIST_OF_PROCESSES[i].get_remaining_brust_time<=quantum_time):
##                
##                    LIST_OF_PROCESSES[i].get_waiting_time()
##                    LIST_OF_PROCESSES[i].get_turn_around_time()
...
##                    LIST_OF_PROCESSES[i].get_remaining_brust_time()
##                   
##                elif(departure_time==0 and LIST_OF_PROCESSES[i].get_arrival_time<=quantum_time and len(LIST_OF_PROCESSES[i])!=0):
...
##                        LIST_OF_PROCESSES[i].get_remaining_brust_time()
##                    else:
##                        READY_QUEUE.append(LIST_OF_PROCESSES[0])
##                
          ...
          if(found==True):
            ...
            #CALCULATING TOTAL WAITING AND TOTAL TURNAROUND TIME
...
#CALCULATING AVERAGE WAITING AND AVERAGE TURN AROUND TIME HERE     
...
def main():
    #CALLING VIRTUAL ROUND ROBIN FUNCTION MADE ABOVE
    #os.system("cls")    
    virtual_round_robin()
